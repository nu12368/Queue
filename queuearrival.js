...
const view_datatable = async (responsedataview, q, day, category) => {
    ...
    document.getElementById("h_name_report").innerText = 'คิววันที่  ' + day + ' / ' + ' คิวที่จองล่วงหน้ารับบริการแล้ว ' + ' / ' + ' แผนก ' + category + ' ' + '( จำนวน ' + _arr_qloop.length + ' คิว )'
...
function getQueueArrivalloop(refresh_token, _page, q) {
    $.getScript("ip.js", function (data, textStatus, jqxhr) {
        var urlipaddress = data.substring(1, data.length - 1);
...
function getQueueArrival(refresh_token) { ////// คิวจองล่วงหน้า
    return new Promise(resolve => {
        $.getScript("ip.js", function (data, textStatus, jqxhr) {
            var urlipaddress = data.substring(1, data.length - 1);
            axios.get(urlipaddress + 'getQueueArrival/' + _objectId + '?_page=1&_limit=100&_sort=1', {
                headers: {
                    'Authorization': refresh_token
                }
            }).then(function (response) {
                ...
                if (looptotle > 1) { ///// คิวมากกว่า loop 100
                    for (i = 0; i < looptotle; i++) {
                        getQueueArrivalloop(refresh_token, _page, 'getQueueArrival/');
                        _page = _page + 1
                    }
    ...
    $(document).ready(function () {
        ...
        $(document).ready(function () {
            ...
            $('#table1_category_list').on('click', 'a.queue', async function (e) { /////ผู้รอรับบริการ แผนก
                ...
                await Getqueueviewcategory(data_click.category, result, 'getQueueArrival/', data._day)
...
async function view_day_category(datetime) {
    ...
    const bookingQueue = await getQueueArrival(result); ////คิวรอรับบริการ
