...
$(document).ready(function () {
    $('#ul_profile').on('click', 'li', function (e) {
...
//     if (asyncCategoryValue.length) {
//      setErrorMessage(false);
//      const [resData] = data.filter((res) => res.key === `${asyncCategoryValue[0]}`);
//      const resQueueNum = await axios.post(`${process.env.REACT_APP_API_URL}/addQueue/${getUserId()}`, {
//       values: resData.category,
//       DPTCODE: resData.DPTCODE,
//       tel: '',
//       name: '',
//      });
//      if (resQueueNum.data.message.queue) {
//       setQueueNum(resQueueNum.data.message.queue);
//       socket.on('addQueue', (values) => {
//        console.log(values);
//       });
